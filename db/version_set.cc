...
bool VersionSet::AddedFiles(const autovector<autovector<VersionEdit*>>& edit_lists) {
  bool res = true;
...
uint64_t VersionSet::LogAndApplyCounter() {
  return log_and_apply_counter_.load();
}

void VersionSet::CreateSyncClient() {
  if (sync_client_ == nullptr) {
    do {
      if (sync_client_ != nullptr) {
        delete sync_client_;
      }
      sync_client_ = new SyncClient(db_options_->channel);
...
Status VersionSet::LogAndApply(
  ...
  if(db_options_->is_rubble && db_options_->is_primary && !db_options_->is_tail){
    ...
    if(db_options_->disallow_flush_on_secondary || (!db_options_->disallow_flush_on_secondary && !is_flush)){
      ...
      if(edit_lists.back().back()->IsFlush()){
        ...
        json mems(ids);
        std::cout << "[LogAndApply] imm " << mems << " " << log_and_apply_counter_.load() << std::endl;
        j_args["MemID"] = mems;
      }
      ...
      ROCKS_LOG_INFO(db_options_->rubble_info_log, 
                    "[Primary] calling logAndApply [%" PRIu64 "] times, type : %s \n",
                     log_and_apply_counter_.load(), type.c_str());
      ...
      for (auto edit_list: edit_lists){
        for (auto e: edit_list) {
          version_edits.emplace_back(e->DebugJSON((int) log_and_apply_counter_, false));
        }
      }
      json j_vec(version_edits);
      j_args["EditList"] = j_vec;
      j_args["Id"] = log_and_apply_counter_.load();
      if(edit_lists.back().size() > 1){
        assert(is_flush);
        // std::cout << j_args.dump(4) << std::endl;
      }
    
      if (AddedFiles(edit_lists)) {
        // We only send version edits generated by flush/compaction jobs
        if (!db_options_->piggyback_version_edits) {
          std::cout << "Calling Sync\n";
          auto start_time = std::chrono::high_resolution_clock::now();
          // This happens during reopen an existing database, e.g., recovery
          CreateSyncClient();
          sync_client_->Sync(j_args.dump());
          std::cout << j_args.dump() << std::endl;
          auto end_time = std::chrono::high_resolution_clock::now();
          auto latency = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
          std::cout << "Sync " << log_and_apply_counter_ << " times latency " << latency << " us type: " << type << std::endl;
        } else {
          ROCKS_LOG_INFO(db_options_->rubble_info_log, "[Primary] : %s \n", j_args.dump(4).c_str());
          db_options_->edits->AddEdit(std::move(j_args.dump()));
        }
      } else {
        std::cout << "Skip Sync " << j_args.dump() << std::endl;
      }
    }
    ...
    for(const auto& edit : edit_lists[0]){
      ...
      std::cout << "[secondary] calling logAndApply version edit " << edit->DebugJSON(edit->GetEditNumber(), false)
                << " log_and_apply_counter_ " << log_and_apply_counter_.load() << " next_fnum " << next_file_number_ << std::endl;
    }
  }
  // RUBBLE END
  int num_edits = 0;
  ...
  Status s = ProcessManifestWrites(writers, mu, db_directory, new_descriptor_log,
                               new_cf_options);
  // printf("[LogAndApply] %lu %lu\nLSM Tree\n", edit_lists.back().back()->GetEditNumber(), log_and_apply_counter_.load());
