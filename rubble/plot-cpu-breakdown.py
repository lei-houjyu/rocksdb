...
data = {"baseline-compaction-primary"   : [0],
        "baseline-dbserver-primary"     : [0],
        "baseline-misc-primary"         : [0],
        "baseline-compaction-secondary" : [0],
        "baseline-dbserver-secondary"   : [0],
        "baseline-misc-secondary"       : [0],
        "rubble-compaction-primary"     : [0],
        "rubble-dbserver-primary"       : [0],
        "rubble-misc-primary"           : [0],
        "rubble-compaction-secondary"   : [0],
        "rubble-dbserver-secondary"     : [0],
        "rubble-misc-secondary"         : [0]}
...
def count_cpu(word, pid_map, mode, job, time):
    ppid = word[1]
    category = mode + '-' + job
...
top_fname = 'top-' + suffix + '.out'
pid_fname = 'pids-' + suffix + '.out'
    ...
    while line:
        ...
        if len(word) == 14 and word[0].isnumeric():
            ...
            if cmd.startswith('rocksdb'):
                count_cpu(word, pid_map, mode, 'compaction', time)
            elif cmd.startswith('grpcpp_sync_ser'):
                count_cpu(word, pid_map, mode, 'dbserver', time)
            else:
                count_cpu(word, pid_map, mode, 'misc', time)
